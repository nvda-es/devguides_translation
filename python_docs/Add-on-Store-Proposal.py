...
_(u"""- Transparent process, to make it simple (for developers) to understand the current state of the Add-on Store, or the state of a submission of a new / updated addon."""),
...
_(u"""However, the metadata for a new submission (add-on release) can be confirmed to match its manifest description."""),
...
_(u"""- NV Access doesn't require a manual review of the add-on (code or user experience) itself before the add-on submission."""),
_(u"""- Source code reviews or audits could exist outside the store. The Sha256 checksum of the `nvda-addon` prevents undetected changes."""),
...
_(u"""- The `.nvda-addon` file accessible via the download URL must continue to match the SHA. NVDA will verify the file has not changed by comparing the checksum (SHA256). This gives users certainty when installing a \"known version\" of an addon. To update or make changes to an add-on, a new unique URL should be used, and a new add-on submission made to the Add-on Store."""),
_(u"""- Allow addon authors to easily revoke a version if it is buggy / no longer supported. Removed releases are no longer presented in the store, halting new installations."""),
...
_(u"""- Use GitHub actions (or other integrations) to automate construction of the data store, and as many of the metadata checks as possible. This will all be open source and extensible by the community."""),
"",_(u"""### Why GitHub Pull Requests?"""),
_(u"""- GitHub is where much of the NVDA development ecosystem is already based."""),
_(u"""- Handles all the of the CRUD ([create, read, update, delete](https://en.wikipedia.org/wiki/CRUD)) for users, authentication is handled, and we can determine how a user relates to an addon repository and what their permissions are for that repository."""),
_(u"""- GitHub PR's keep a record of the outcomes of automated checks. They also facilitate discussion should there be any confusion or disagreement with the outcome."""),
...
_(u"""  - While this is technically not necessary, it provides a good separation from implementation. If we wished to change our storage mechanism, we would not be breaking old versions of NVDA."""),
"",_(u"""## `addon-store-submission` GitHub Repository"""),
...
_(u"""If a newer add-on release is removed (in response to a critical bug being found) NVDA can fall back on a prior add-on release."""),
...
"",_(u"""### Metadata format"""),
_(u"""For a full description of the schema see the [_validate/addonVersion_schema.json file](https://github.com/nvaccess/validateNvdaAddonMetadata/blob/main/_validate/addonVersion_schema.json)."""),
...
"",_(u"""### Other notes"""),
_(u"""- By using a git repository and PR process, `git blame` and `git log` can be used to get more context about addons listed in the store. For instance:"""),
...
"",_(u"""### Overview"""),
"",_(u"""For each version of NVDA, the meta-data of the most recent (the highest version number) of each Addon is automatically added, based on the data in 'addon-store-submission'."""),
...
"",_(u"""The simplicity of this is that the NV Access server can just forward these files on directly when asked \"what are the latest Addons for NVDA API Version X\" or \"What is the latest version of Addon-ID for NVDA API Version X\"."""),
_(u"""Using the NV Access server as the endpoint for this is important in case the implementation has to change or be migrated away from GitHub for some reason."""),
"",_(u"""## Suffix"""),
