...
"",_(u"""* Latest version: February 2018 for NVDA 2018.1"""),
...
"",_(u"""* manifest.ini to store manifest information such as add-on name and author."""),
...
"",_(u"""Note: for this chapter, we will not use the actual add-on packages. Instead, we'll use plugin folders - a number of subdirectories in your NVDA user configuration folder (available from Start Menu/Screen if NVDA is installed) to store our example Python files."""),
...
"",_(u"""To run your example add-ons from this chapter, open your NVDA user configuration directory (from Start Menu/Screen, look for Explore NVDA user configuration folder\" item). Then paste your .py file to the appropriate folder: appModules folder for app module examples, and globalPlugins folder for global plugins."""),
...
"",_(u"""The below code, from NVDA developer Guide, gives a short example of a typical app module: play a short beep when switching to Notepad. Put the below code in notepad.py, which in turn should be placed in appModules folder in your user configuration folder in order for it to run."""),
...
"",_(u"""* Get to know NVDA: it is important that you become familiar with NVDA commands, concepts and tips. Subscribe to NVDA users groups to learn more about NVDA and learn about how NVDA works, as you'll be extending it via your add-ons."""),
...
_(u"""This cannot be done easily. One handy module for this purpose is six, which allows running Python 2 and 3 code. NvDA itself uses Python 2 as WXPython uses Python 2, and until WXPython is rewritten to support Python 3, you cannot run Python 3 code in NVDA."""),
...
_(u"""* Make sure you know how to communicate with your equipment - ports, USB ID's, Bluetooth addresses, serial port settings and so on."""),
...
_(u"""| I wish to do something whenever configuration profiles are changed | config.configProfileSwitch | You need to register a function to listen to this action, then let this function do something when profiles are changed. |"""),
