...
"",_(u"""Latest version: August 2021 for NVDA 2021.2"""),
"",_(u"""---"""),
...
_(u"""    - [Special note on NVDA backward compatibility](#user-content-special-note-on-nvda-backward-compatibility)"""),
...
_(u"""    - [A very important note about control types module changes in NVDA 2021.2](#user-content-a-very-important-note-about-control-types-changes-in-nvda-2021.2)"""),
...
_(u"""    - [System Requirements](#user-content-system-requirements)"""),
...
_(u"""    - [Submitting your add-on for review](#user-content-submitting-your-add-on-for-review)"""),
_(u"""        - [Add-on review checklist](#user-content-add-on-review-checklist)"""),
...
"",_(u"""### Special note on NVDA backward compatibility"""),
"",_(u"""To modernize NVDA source code and to respond to screen reader usage changes, NV Access has adopted an annual backward compatibility policy in 2020. For each calendar year, the first major version (year.1) is designated \"backwards incompatible\" release where changes affecting add-ons will be incorporated. These include changes to names of functions and classes, as well as removing deprecated code. Because these changes will affect many add-ons, developers must test their add-ons for compatibility once the first beta of the backwards incompatible version of NVDA is released."""),
"",_(u"""List of backwards incompatible NVDA releases and their highlights:"""),
"",_(u"""* 2019.3: Python 3"""),
_(u"""* 2021.1: dependency updates"""),
_(u"""* 2022.1: control types refactor"""),
"",_(u"""Unless otherwise stated, this guide will assume latest backwards incompatible NVDA when giving code examples (as of August 2021, 2021.1 is assumed). Exceptions will be documented in appropriate places."""),
...
"",_(u"""### A very important note about control types module changes in NVDA 2021.2"""),
"",_(u"""NVDA 2021.2 introduces control types refactor that changes how control roles and sates are specified. In older NvDA releases, control roles are written as controlTypes.ROLE_*, and states are written as controlTypes.STATE_*. With control types refactor, roles and states must be written as controlTypes.Role.* and controlTypes.State.*, respectively. For example:"""),
"",_(u"""* Editable text role:"""),
_(u"""    * 2021.1 and earlier: controlTypes.ROLE_EDITABLETEXT"""),
_(u"""    * 2021.2 and later: controlTypes.Role.EDITABLETEXT"""),
_(u"""* Checkable state:"""),
_(u"""    * 2021.1 and earlier: controlTypes.STATE_CHECKABLE"""),
_(u"""    * 2021.2 and later: controlTypes.State.CHECKABLE"""),
"",_(u"""The older way of specifying control roles and states will be removed in NVDA 2022.1. Unless otherwise stated, this guide will use the newer style."""),
"",_(u"""## Add-on Basics"""),
...
_(u"""* Python:"""),
_(u"""    - Python 3.7 series, version 3.7.9 32-bit for Windows: https://www.python.org/downloads/release/python-379/"""),
...
_(u"""    - `api.copyToClip(text to be copied, optionally notify success or failure)`: copies text to the clipboard and optionally let NVDA notify the user about this operation."""),
...
_(u"""    - `eventHandler.requestEvents(event to be requested, process ID, window class name for the control)`: Allows NVDA to listen to specific events for certain controls while using another app."""),
_(u"""* Extension points (extensionPoints): provides a way to let add-ons and other modules define and respond to specific action such as profile switches, actions in an add-on and so on. The following extension points are defined:"""),
...
_(u"""* Java support (JABHandler.py, JAB objects): A collection of methods used for supporting JAB (Java Access Bridge) subsystem used for Java applications."""),
...
_(u"""    - `touchHandler.touchSupported()`: returns if the system supports touch interaction."""),
...
_(u"""    - `ui.message(message to be spoken/brailled, speech priority, optional braille message)`: Speaks or brailles the message (a string surrounded by quotes). Optionally, speech priority can be specified to interrupt what the speech synthesizer is saying when announcing the message, as well as output a different message on braille displays."""),
...
"",_(u"""```python"""),
_(u"""	def getSliderValue(self):"""),
_(u"""		from controlTypes import Role # It is possible to import from within a method."""),
_(u"""		fg = api.getForegroundObject()"""),
_(u"""		slider = fg.lastChild"""),
_(u"""		if slider.role == Role.SLIDER: return slider.firstChild.value"""),
_(u"""```"""),
...
_(u"""		if obj.windowClassName == \"TForm\": obj.role = controlTypes.Role.WINDOW"""),
_(u"""```"""),
...
"",_(u"""Q. I need to manipulate environment variables such as system path."""),
"",_(u"""An effective way to do this is prepending a desired string (such as the path to an executable you need to run) to existing environment variable value. This allows your add-on to work with modified values without breaking NVDA and/or Windows components while NVDA is running."""),
...
_(u"""		lineNumList = api.getStatusBar().getChild(1).name.split()"""),
_(u"""		lineNum = lineNumList[0]+lineNumList[1]"""),
...
_(u"""		eventHandler.requestEvents(\"valueChange\", self.processID, \"MessengerWindow\")"""),
_(u"""```"""),
...
"",_(u"""### Submitting your add-on for review"""),
"",_(u"""If you would like to submit your add-on for distribution on community add-ons website, you must request a review from a member of the add-ons community as follows:"""),
"",_(u"""1. If you haven't, subscribe to NVDA Add-ons mailing list."""),
_(u"""2. Ask the community to review your add-on. Be sure to specify name, author, brief description of the add-on, and public code repository if possible."""),
_(u"""3. Community members will review and comment on your add-on, including license and copyright, user experience, documentation, and security."""),
_(u"""4. If your add-on is accepted for community add-ons website distribution, a maintainer of the community add-ons website will register your add-on. This includes creating a new entry for your add-on and preparing localization support."""),
_(u"""5. Optionally, after your add-on is accepted, you can ask the community for more detailed reviews. These can include improving messages, GUI layout, advanced security audit, compatibility testing, and porting to newer Python versions."""),
"",_(u"""#### Add-on review checklist"""),
"",_(u"""Your add-on:"""),
"",_(u"""1. Must be licensed under GNU General Public license (GPL) version 2 or later or equivalent which allows a GPL software such as NVDA to incorporate your code while it runs."""),
_(u"""2. Must be written in Python 3. Compatibility with Python 2 is optional but not recommended starting with NVDA 2021.1 base API."""),
_(u"""3. Must be compatible with latest base API release (as of May 2021, base API is 2021.1)."""),
_(u"""4. Messages to be presented to users should be made translatable (use _() Gettext function to make messages translatable). Be sure to accompany translatable messages with comments for translators."""),
"",_(u"""## Advanced Code Examples and Features"""),
"",_(u"""This chapter is a work in progress. If there is something you would like to see here, or if you have a comment or correction, please contact one of the maintainers, or ask for it on the add-ons mailing list."""),
...
"",_(u"""class GlobalPlugin(globalPluginHandler.GlobalPlugin):"""),
"",_(u"""	@script("""),
_(u"""		gesture=\"kb:nvda+shift+t\",  # Configure the key"""),
_(u"""		description=\"Run an add-on guide example\"  # NVDA input help"""),
_(u"""	)"""),
...
_(u"""```"""),
"",_(u"""### Using The Log"""),
"",_(u"""There are more in-depth things you can do with the log than what will be shown here."""),
_(u"""However, most of the time, all you want to do is write a basic message into the log. To do that, you can import NVDA's log singleton, and call its methods just like any others."""),
"",_(u"""```python"""),
_(u"""import globalPluginHandler"""),
_(u"""from scriptHandler import script"""),
_(u"""from logHandler import log  # This is what you need for logging"""),
_(u"""from datetime import date"""),
"",_(u"""class GlobalPlugin(globalPluginHandler.GlobalPlugin):"""),
"",_(u"""	@script("""),
_(u"""		gesture=\"kb:nvda+shift+l\",  # Configure the key"""),
_(u"""		description=\"Run an add-on guide example\"  # NVDA input help"""),
_(u"""	)"""),
_(u"""	def script_captainsLog(self, gesture):"""),
_(u"""		today = date.today().strftime(\"%Y.%m.%d\")"""),
_(u"""		log.info(f\"NVDA log. Earth date, {today}.\")"""),
_(u"""		log.warning(\"These are the add-ons of the screen reader NVDA.\")"""),
_(u"""		log.debugWarning(\"Its continuing mission. To seek out new opportunities to improve lives!\")"""),
_(u"""		log.debug(\"To empower users!\")"""),
_(u"""		log.error(\"and to boldly access software that no screen reader has made accessible before!\")"""),
_(u"""```"""),
"",_(u"""If you run the above in the NVDA scratchpad or a global plugin, and call it by pressing the Shift+NVDA+L key sequence, you will receive between zero and five log entries, depending on how your NVDA logging level is configured in general settings. For example, if your log level is set to \"debug\", you will find them all there, but only four of them if your log level is set to \"debug warning\"."""),
"",_(u"""Note: while developing add-ons, it is usually wise to have the most debugging that you can, so you can gather information when things go wrong. For that reason, you may want to set your logging level to \"debug\"."""),
"",_(u"""### Threading"""),
"",_(u"""#### Introduction"""),
...
_(u"""| Check if the object's role is what you want | obj.role == controlTypes.Role.* | Role.* can be any role you are looking for. |"""),
...
_(u"""| Is a checkbox checked | controlTypes.State.CHECKED in obj.states | obj.states is a set. First, verify that the role is a checkbox. |"""),
...
_(u"""| Automation ID for a UIA element | obj.UIAAutomationID | First, check if the object is a UIA control. |"""),
_(u"""| Framework used to generate this UIA object | obj.UIAElement.cachedFrameworkID | The GUI framework used to program this object. Commonly encountered frameworks are Direct UI, Windows Presentation Foundation (WPF) controls with UIA enabled, XAML (eXtensible Application Markup Language) and Microsoft Edge. |"""),
...
_(u"""| Path to the executable for the foreground object | api.getForegroundObject().appModule.appPath | Obtains the path to the executable for any object. |"""),
_(u"""| Provide information about a script in one go | scriptHandler.script(description, one or more gestures, category) | This is a decorator. Typically you would write: @scriptHandler.script followed by the script information. Gestures can be a single gesture (gesture=string) or a list (gestures=[gestures]). |"""),
_(u"""| Assign a keyboard command to a script | @scriptHandler.script(gesture=keyboardCommand) | Keyboard gestures start with a \"kb:\". |"""),
...
_(u"""| I want to assign Control+Alt+number row to a script | @scriptHandler.script(gestures=[f\"kb:control+alt+{i}\" for i in range(10)]) | This uses a combination of a decorator and formatted string literals (f strings) defined inside a list comprehension. |"""),
_(u"""| providing input help message | @scriptHandler.script(description=inputHelpMessage) | Effectively, a script's description (assigned to its docstring) is treated as its input help message. |"""),
...
_(u"""| Is touchscreen support available | touchHandler.touchSupported() | If true, touch support is active and available. |"""),
